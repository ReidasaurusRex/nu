  ...
  def create_like
    ...
    if @like.save
      ...
      unless @newsfeed_item.belongs_to_profile?(@profile)
        @newsfeed_item.source.notifications.create(link: "/profiles/#{@newsfeed_item.source.id}/newsfeed_items/#{@newsfeed_item.id}", text: "#{@profile.full_name} liked your post")
      end
